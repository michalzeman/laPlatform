...
import org.apache.commons.cli.{BasicParser, CommandLine, HelpFormatter, Options, ParseException, Option => CliOption}
...
class CliParams(args: Array[String]) {
  ...
  protected val options: Options = buildOptions
  protected val parser = new BasicParser
  protected val formatter = new HelpFormatter

  protected val cmd: CommandLine = parseCli()

  protected def buildOptions: Options = {
    val options = new Options
    val dataDirOpt = buildOption("d", "dataDir", hasArg = true, description = "Directory where are all data stored!")
  ...
  /**
    ...
    */
  protected def buildOption(opt: String, longOpt: String, hasArg: Boolean, description: String): CliOption =
    new CliOption(opt, longOpt, hasArg, description)
